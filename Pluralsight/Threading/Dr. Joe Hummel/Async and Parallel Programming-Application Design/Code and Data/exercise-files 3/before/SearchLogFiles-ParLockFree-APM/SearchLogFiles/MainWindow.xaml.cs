...
//
// Parallel Search version -- lock-free approach + asynchronous File I/O using APM pattern
//
...
// (one task per file) as well as take advantage of .NET's asynchronus file I/O support.
//
	...
	public partial class MainWindow : Window
	{
		...
		private int SearchFiles(List<string> filenames, string pattern)
		{
			...
			foreach (string f in filenames)
			{
				...
				//
				string fn = f;  // capture loop variable for async I/O task:

				//
				// read the file, but let's use APM pattern + FromAsync to do async I/O:
				//
				...
				// open file, note "async" arg at end has to be "true":
				FileStream fs = new FileStream(fn, FileMode.Open, FileAccess.Read, FileShare.Read, bytesToRead, true /*async*/);

				// Create a facade task to represent & start async I/O operation:
				Task<int> doRead = Task<int>.Factory.FromAsync(fs.BeginRead, fs.EndRead, buf, 0, bytesToRead, null);

				// wait for read task to finish, and when it does, search the data:
				Task<int> doSearch = doRead.ContinueWith<int>((antecedent) =>
					{
						fs.Close();  // make sure file gets closed:

						int bytesActuallyRead = doRead.Result;  // wait, havest result (# of bytes actually read):

						if (bytesActuallyRead < bytesToRead)
						  throw new ApplicationException("Async I/O failed to read file?!");
						...
						string contents = encoding.GetString(buf);
